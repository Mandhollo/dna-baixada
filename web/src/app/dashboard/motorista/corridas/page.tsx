...
/* ─── Icon map for CorridaTipo (lucide-react, no emojis) ─── */
...
const TIPO_OPCOES: { value: CorridaTipo | 'todos'; label: string }[] = [
  { value: 'todos', label: 'Todos os tipos' },
  ...CORRIDA_TIPOS.map((t) => ({ value: t.value, label: t.label })),
];
...
export default function CorridasMotoristaPage() {
  ...
  const [filtroTipo, setFiltroTipo] = useState<CorridaTipo | 'todos'>('todos');
  ...
  const fetchCorridas = useCallback(async () => {
    ...
    const { data, error, count } = await supabase
      .from('corridas')
      .select('*, passageiro:profiles!passageiro_id(nome)', { count: 'exact' })
  ...
  }, [corridas, filtroStatus, filtroTipo]);
  ...
  const stats = useMemo(() => {
    ...
    return { total, finalizadas, canceladas };
  }, [corridas]);
  ...
  useEffect(() => {
    ...
    const handler = () => setShowTipoDropdown(false);
        ...
        >
          ...
          <div className="relative mt-3 inline-block" onClick={(e) => e.stopPropagation()}>
            <button
              ...
              className="flex items-center gap-2 rounded-full border border-border bg-surface-elevated px-4 py-2 text-xs font-semibold text-foreground-secondary transition hover:border-border-strong"
            >
              <Car size={14} />
              {TIPO_OPCOES.find((o) => o.value === filtroTipo)?.label ?? 'Todos os tipos'}
              <ChevronDown
                size={14}
                className={`transition-transform ${showTipoDropdown ? 'rotate-180' : ''}`}
              />
            </button>
            ...
            <AnimatePresence>
              {showTipoDropdown && (
                <motion.div
                  ...
                  className="absolute left-0 top-full z-20 mt-2 w-56 origin-top overflow-hidden rounded-xl border border-border bg-surface-elevated shadow-xl"
                >
                  {TIPO_OPCOES.map((opt) => {
                    const TipoIcon =
                      opt.value !== 'todos' ? TIPO_ICON[opt.value] : Car;
                    return (
                      <button
                        key={opt.value}
                        onClick={() => {
    ...
    >
      ...
      <div className="mt-3 space-y-1">
        ...
        {corrida.destino_endereco && (
          <div className="flex items-start gap-2">
            <Navigation size={13} className="mt-0.5 shrink-0 text-accent2" />
            <p className="truncate text-xs text-foreground-secondary">
              {corrida.destino_endereco}
            </p>
          </div>
      ...
      {/* Bottom row: date + passengers + distance */}
      <div className="mt-3 flex items-center gap-3 text-[11px] text-foreground-muted">
        ...
        <span className="flex items-center gap-1">
          <Users size={11} />
          {corrida.passageiros} passageiro{corrida.passageiros !== 1 ? 's' : ''}
        </span>
    ...
    <nav className="fixed bottom-0 left-0 right-0 z-30 border-t border-border bg-surface-elevated shadow-lg">
      <div className="mx-auto flex max-w-5xl items-center justify-around py-2">
        ...
        <NavItem
          ...
          label="Corridas"
          active
          onClick={() => router.push('/dashboard/motorista/corridas')}
        />
        <NavItem
          icon={<Wallet size={20} />}
          label="Financeiro"
